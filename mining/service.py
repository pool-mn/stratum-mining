...
class MiningService(GenericService):
    ...
    def submit(self, worker_name, work_id, extranonce2, ntime, nonce):
        ...
        if str(ip) in Interfaces.worker_manager.worker_log['ip_difficulty']:
            (ip_count, ip_quant, ip_is_banned, ip_end_ban_time) = Interfaces.worker_manager.worker_log['ip_difficulty'][str(ip)]
        else:
            (ip_count, ip_quant, ip_is_banned, ip_end_ban_time) = (0., 0, False, 0)

        # If user id ip performance ban
        if ip_is_banned:
            if submit_time < ip_end_ban_time:
                log.info("Worker is banned by ip: IP %s", str(ip))
                raise SubmitException("Worker is temporarily banned by ip")
            else:
                ip_is_banned = False
                ip_end_ban_time = 0

        if worker_name in Interfaces.worker_manager.worker_log['perf_difficulty']:
            (perf_quant, perf_count, perf_is_banned, perf_end_ban_time) = Interfaces.worker_manager.worker_log['perf_difficulty'][worker_name]
        else:
            (perf_quant, perf_count, perf_is_banned, perf_end_ban_time) = (0, 0, False, 0)

        # If user in performance ban
        if perf_is_banned:
            # Check end ban time
            if submit_time < perf_end_ban_time:
                raise SubmitException("Worker is temporarily banned by performance")
            else:
                perf_is_banned = False
                perf_end_ban_time = 0
        ...
        log.debug("%s (%d, %d, %s, %s, %d) %0.2f%% work_id(%s) job_id(%s) diff(%f) performance(%d %d %s %d)" % (worker_name, valid, invalid, is_banned, is_ext_diff, last_ts, percent, work_id, job_id, difficulty, perf_quant, perf_count, perf_is_banned, perf_end_ban_time))
        if not is_ext_diff:    
        ...
        try:
            ...
            Interfaces.worker_manager.worker_log['perf_difficulty'][worker_name] = (perf_quant, perf_count, perf_is_banned, perf_end_ban_time)
            Interfaces.worker_manager.worker_log['ip_difficulty'][str(ip)]  = (ip_count, ip_quant, ip_is_banned, ip_end_ban_time)
        ...
        #############                   Ip ban                    ##############
        # Calc count valid share for ip by current by current time interval
        ip_curr_quant = submit_time // settings.IPBAN_INTERVAL
        if ip_curr_quant != ip_quant:
            ip_quant = ip_curr_quant
            ip_count = difficulty
        else:
            ip_count += difficulty

        # Check conditional for ban by performance
        if perf_count >= settings.IPBAN_MAX_SHARES and settings.IPBAN_ENABLED:
            ip_is_banned = True
            ip_end_ban_time = (ip_curr_quant + 1 + settings.IPBAN_BAN_TIME) * settings.IPBAN_INTERVAL
            log.debug("Worker has a great performance (more %s per minute): %s BANNED BY IP %s!" % (perf_count, worker_name, str(ip)))

        ##############              Performance ban               ##############
        # Calc count valid share by current by current time interval
        perf_curr_quant = submit_time // settings.PERFBAN_INTERVAL
        if perf_curr_quant != perf_quant:
            perf_quant = perf_curr_quant
            perf_count = difficulty
        else:
            perf_count += difficulty

        # Check conditional for ban by performance
        if perf_count >= settings.PERFBAN_MAX_SHARES and settings.PERFBAN_ENABLED:
            perf_is_banned = True
            perf_end_ban_time = (perf_curr_quant + 1 + settings.PERFBAN_BAN_TIME) * settings.PERFBAN_INTERVAL
            log.debug("Worker has a great performance (more %s per minute): %s BANNED!" % (perf_count, worker_name))

        log.debug("Performance(%d %d %s %d)" % (perf_quant, perf_count, perf_is_banned, perf_end_ban_time))
        log.debug("Ip(%s %s %d %s %d)" % (str(ip), ip_count, ip_quant, ip_is_banned, ip_end_ban_time))
        ...
        if perf_is_banned:
            raise SubmitException("Worker is temporarily banned, hashrate is above the limit")
