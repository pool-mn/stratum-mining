...
class BlockTemplate(halfnode.CBlock):
    ...
    def fill_from_rpc(self, data):
        ...
        if settings.CHARITY_ENABLED is True:
            if settings.CHARITY_ISDAEMONAMOUNT is True:
                charity_value = data['charityvalue']
            else:
                charity_value = settings.CHARITY_AMOUNT

            if settings.CHARITY_ISCOINBASEINCLUDESCHARITY is True:
                miner_reward = data['coinbasevalue'] - charity_value
            else:
                miner_reward = data['coinbasevalue']
        else:
            charity_value = 0
            miner_reward = data['coinbasevalue']

        if settings.COINDAEMON_Reward == 'POW':
            coinbase = CoinbaseTransactionPOW(self.timestamper, self.coinbaser, miner_reward, charity_value,
                                              data['coinbaseaux']['flags'], data['height'],
                                              settings.COINBASE_EXTRAS)
        else:
            coinbase = CoinbaseTransactionPOS(self.timestamper, self.coinbaser, miner_reward, charity_value,
                                              data['coinbaseaux']['flags'], data['height'],
