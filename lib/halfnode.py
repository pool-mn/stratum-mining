       ...
       def calc_scrypt(self):
         ...
         def calc_x17(self):
             if self.x17 is None:
                ...
                r.append(struct.pack("<I", self.nBits))
                r.append(struct.pack("<I", self.nNonce))
                self.x17 = uint256_from_str(crypto_hash.getPoWHash(''.join(r)))
             return self.x17
    elif settings.COINDAEMON_ALGO == 'fresh':
         def calc_fresh(self):
             if self.fresh is None:
                ...
                r.append(struct.pack("<I", self.nBits))
                r.append(struct.pack("<I", self.nNonce))
                self.fresh = uint256_from_str(fresh_hash.getPoWHash(''.join(r)))
             return self.fresh
    elif settings.COINDAEMON_ALGO == 'scrypt-jane':
